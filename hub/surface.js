...
async function openEmmaWorld() {
  ...
  let clockHtml = '<div class="emma-clock-standalone" id="emma-clock-solo">' +
    '<span class="emma-clock-spark">\u2726</span><span id="emma-clock-time"></span><span class="emma-clock-spark">\u2726</span></div>';
  if (window._emmaItinerary && window._emmaItinerary.length) {
    // itinerary shows separately; clock stays standalone
  ...
  el.innerHTML =
    ...
    '.emma-itin{position:absolute;bottom:20vh;left:4vw;z-index:4;' +
    'background:rgba(26,0,17,0.6);backdrop-filter:blur(8px);border:1px solid rgba(255,110,199,0.3);' +
    'border-radius:14px;padding:1em 1.3em;max-width:30vw;}' +
  ...
  function updateEmmaClock() {
    ...
    const c = document.getElementById('emma-clock-time');
