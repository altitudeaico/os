...
var elsieState = {
  photos: [], artwork: [], cheer: [], music: [], highlights: [], reels: [],
  reelSel: 0, reelVideo: null, reelIdx: -1, reelResumeMusic: false,
...
async function elsieLoadContent() {
  ...
  try {
    var res = await Promise.all([
      ...
      // Approved reels only: the same watch_catalog feed as Watch, so approval puts a reel in both places.
      fetch(ELSIE_API + '/watch_catalog?select=id,title,video_url,poster_url,duration_seconds&kind=eq.edit&person=eq.Elsie&order=dated_at.desc,title.asc', { headers: hdr }).then(function (r) { return r.json(); }).catch(function () { return []; }),
    ]);
    return {
      ...
      reels: Array.isArray(res[5]) ? res[5] : [],
    };
  } catch (e) {
    return { photos: ELSIE_FALLBACK_PHOTOS, artwork: [], cheer: [], music: [], highlights: [], reels: [] };
  }
}
...
async function openElsieWorld() {
  ...
  elsieState.reels = content.reels;

  el.innerHTML = elsieBuildHTML();
  if (typeof fosFitScan === 'function') fosFitScan(el.querySelectorAll('.elsie-slide'));
  elsieInjectStyles();
  elsieStartSparkles();
  ...
  elsieFocusMain(elsieState.reels.length ? 'widget-reels' : 'widget-gallery');
...
function elsieBuildHTML() {
  ...
  var reelCount = elsieState.reels.length;
  var reelThumb = reelCount ? (elsieState.reels[0].poster_url || '') : '';
  ...
  return '' +
    ...
    '<div class="elsie-stack">' +
      (reelCount
        ? '<div class="elsie-card elsie-reels-card" id="widget-reels">' +
            '<div class="elsie-card-head">My Reels <span class="elsie-card-open">OK</span></div>' +
            '<div class="elsie-widget-row"><div class="elsie-widget-thumb elsie-reel-thumb" style="background-image:url(' + reelThumb + ');"></div><div class="elsie-widget-meta" id="elsie-reels-meta">' + reelCount + ' reel' + (reelCount === 1 ? '' : 's') + '</div></div>' +
          '</div>'
        : '') +
    ...
    '<div class="elsie-reel-player" id="elsie-reel-player" style="display:none;"></div>' +
...
function elsieInjectStyles() {
  ...
  s.textContent =
    ...
    '.elsie-reel-thumb{width:clamp(34px,2.6vw,48px);height:clamp(58px,4.6vw,84px);border-radius:8px;}' +
    '.elsie-reel-piece{width:12vw;max-width:220px;}' +
    '.elsie-reel-piece .elsie-gallery-frame{aspect-ratio:9/16;}' +
    '.elsie-reel-player{position:fixed;inset:0;z-index:20;background:#000;overflow:hidden;}' +
    '.elsie-reel-bg{position:absolute;inset:-4vw;background:center/cover no-repeat;filter:blur(40px) brightness(0.35);}' +
    '.elsie-reel-player video{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;}' +
    '.elsie-reel-title{position:absolute;left:4vw;bottom:3vw;color:#fff;font-size:1.3vw;font-weight:700;text-shadow:0 0.2vw 1vw rgba(0,0,0,0.8);transition:opacity 0.4s;}' +
    '.elsie-reel-state{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);color:#fff;font-size:4vw;opacity:0;transition:opacity 0.25s;}' +
...
function elsieShowPhoto(idx) {
  ...
  if (typeof fosFitScan === 'function') fosFitScan(slides);
  var n = slides.length;
  idx = ((idx % n) + n) % n;
...
function elsieFocusMain(focusId) {
  ...
  fm.reset();
  var reelsW = document.getElementById('widget-reels');
  ...
  if (reelsW && elsieState.reels.length) fm.register('widget-reels', reelsW, function () { elsieOpenReels(); });
  ...
  var startId = focusId;
  if (startId === 'widget-reels' && !elsieState.reels.length) startId = 'widget-gallery';
...
/* ══ EXPANDED: MY REELS (approved edits, same feed as Watch) ══ */
function elsieOpenReels() {
  var ov = document.getElementById('elsie-overlay');
  var reels = elsieState.reels;
  var itemsHtml = reels.map(function (r, i) {
    var dur = r.duration_seconds ? Math.round(r.duration_seconds) + 's' : '';
    return '<div class="elsie-gallery-piece elsie-reel-piece" data-idx="' + i + '">' +
      '<div class="elsie-gallery-frame" style="background-image:url(' + (r.poster_url || '') + ');"></div>' +
      '<div class="elsie-gallery-title">' + (r.title || 'Reel') + '</div>' +
      (dur ? '<div class="elsie-gallery-caption">' + dur + '</div>' : '') +
      '</div>';
  }).join('');
  ov.innerHTML =
    '<div class="elsie-ov-head">My Reels</div>' +
    '<div class="elsie-ov-sub">' + reels.length + ' reel' + (reels.length === 1 ? '' : 's') + '</div>' +
    '<div class="elsie-gallery-grid" id="elsie-reels-grid">' + itemsHtml + '</div>' +
    '<div class="elsie-ov-foot">\u25c0\u25b6  Browse    OK  Play    Back  Return</div>';
  ov.style.display = 'flex';

  elsieState.nav.push('elsie-reels', 'widget-reels', null, function () { ov.style.display = 'none'; });

  var fm = elsieState.focus;
  fm.reset();
  ov.querySelectorAll('.elsie-reel-piece').forEach(function (el) {
    var idx = parseInt(el.getAttribute('data-idx'), 10);
    // play() must run synchronously inside the OK key handler (TV autoplay rule)
    fm.register('reel-' + idx, el, function () { elsiePlayReel(idx); });
  });
  if (elsieState.reelSel >= reels.length) elsieState.reelSel = 0;
  fm.focus('reel-' + elsieState.reelSel);
  elsieSetHint('\u25c0\u25b6  Browse    OK  Play    Back  Return');
}

function elsiePlayReel(idx) {
  var r = elsieState.reels[idx];
  var pl = document.getElementById('elsie-reel-player');
  if (!r || !pl) return;
  if (elsieState.reelVideo) { try { elsieState.reelVideo.pause(); } catch (e) {} }
  if (elsieState.reelIdx < 0) {
    // Pause Elsie's music while a reel plays; resume it when the player closes.
    elsieState.reelResumeMusic = !!elsieState.playing;
    if (elsieState.playing && elsieState.audio) { try { elsieState.audio.pause(); } catch (e) {} elsieState.playing = false; elsieUpdatePlayIcon(); }
  }
  pl.innerHTML =
    '<div class="elsie-reel-bg" style="background-image:url(' + (r.poster_url || '') + ');"></div>' +
    '<div class="elsie-reel-title" id="elsie-reel-title">' + (r.title || '') + '</div>' +
    '<div class="elsie-reel-state" id="elsie-reel-state">\u275a\u275a</div>';
  var v = document.createElement('video');
  v.setAttribute('playsinline', '');
  v.preload = 'auto';
  if (r.poster_url) v.poster = r.poster_url;
  v.src = r.video_url;
  v.onended = function () {
    if (idx < elsieState.reels.length - 1) { elsiePlayReel(idx + 1); return; }
    var restored = elsieState.nav.back();
    if (restored && restored.view === 'elsie-reels') { elsieState.focus.focus('reel-' + idx); }
  };
  pl.appendChild(v);
  pl.style.display = 'block';
  elsieState.reelVideo = v;
  elsieState.reelIdx = idx;
  elsieState.reelSel = idx;
  var p = v.play();
  if (p && typeof p.catch === 'function') p.catch(function () {});
  if (elsieState.nav.currentView() !== 'elsie-reel-player') {
    elsieState.nav.push('elsie-reel-player', 'reel-' + idx, null, function () { elsieStopReel(); });
  }
  elsieSetHint('OK  Pause    \u25c0\u25b6  \u00b110s    Back  Close');
  setTimeout(function () { var t = document.getElementById('elsie-reel-title'); if (t) t.style.opacity = '0'; }, 3500);
}

function elsieStopReel() {
  var pl = document.getElementById('elsie-reel-player');
  var v = elsieState.reelVideo;
  if (v) { try { v.pause(); v.removeAttribute('src'); v.load(); } catch (e) {} }
  elsieState.reelVideo = null;
  elsieState.reelIdx = -1;
  if (pl) { pl.style.display = 'none'; pl.innerHTML = ''; }
  if (elsieState.reelResumeMusic && elsieState.audio && !elsieState.playing) {
    elsieState.audio.play().catch(function () {}); elsieState.playing = true; elsieUpdatePlayIcon();
  }
  elsieState.reelResumeMusic = false;
  elsieSetHint('\u25c0\u25b6  Browse    OK  Play    Back  Return');
}

function elsieToggleReel() {
  var v = elsieState.reelVideo; if (!v) return;
  var st = document.getElementById('elsie-reel-state');
...
function elsieUpdateHint() {
  ...
  if (id === 'widget-reels') elsieSetHint('OK  Watch my reels');
  else if (id === 'widget-gallery') elsieSetHint('OK  Open gallery');
...
function elsieStartPoll() {
  ...
  elsieState.pollTimer = setInterval(async function () {
    ...
    if (elsieState.nav.currentView() === 'elsie-main') {
      ...
      elsieState.highlights = content.highlights;
      elsieRenderLatest();
      if (content.reels.length === elsieState.reels.length || !content.reels.length) { elsieState.reels = content.reels.length ? content.reels : elsieState.reels; }
      else { elsieState.reels = content.reels; var rm = document.getElementById('elsie-reels-meta'); if (rm) rm.textContent = content.reels.length + ' reel' + (content.reels.length === 1 ? '' : 's'); }
    }
  }, 25000);
}
...
function elsieCleanup() {
  ...
  if (elsieState.reelVideo) { try { elsieState.reelVideo.pause(); } catch (e) {} elsieState.reelVideo = null; }
  elsieState.reelIdx = -1; elsieState.reelResumeMusic = false;
...
function elsieHandleKey(key, code) {
  ...
  if (isBack) {
    ...
    var restored = elsieState.nav.back();
    if (restored && restored.view === 'elsie-reels') {
      elsieState.focus.focus(restored.focusId || 'reel-0');
      return true;
    }
  ...
  if (view === 'elsie-reel-player') {
    var rv = elsieState.reelVideo;
    if (key === 'Enter' || code === 13 || code === 23) { elsieToggleReel(); return true; }
    if (rv && (key === 'ArrowLeft' || code === 37)) { rv.currentTime = Math.max(0, rv.currentTime - 10); return true; }
    if (rv && (key === 'ArrowRight' || code === 39)) { rv.currentTime = Math.min(rv.duration || 0, rv.currentTime + 10); return true; }
    return true;
  }
  if (view === 'elsie-reels' && (key === 'ArrowLeft' || code === 37 || key === 'ArrowRight' || code === 39)) {
    var hr = elsieState.focus.handleKey(key, code);
    if (hr) { var mr = elsieState.focus.currentId.match(/^reel-(\d+)$/); if (mr) elsieState.reelSel = parseInt(mr[1], 10); }
    return hr;
  }
